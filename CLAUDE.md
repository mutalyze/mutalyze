...
These are real rules for developing ruleguard itself — not a demo. The point is
to work under them for a while and then run `ruleguard check` on the session, so
...
- Run `./.venv/bin/python tests/make_fixture.py` after changing anything in `ruleguard/` and before committing.
- Use `rg` (not `grep`) for searching the codebase.

## Code

- No `print()` for debugging in `ruleguard/` — the CLI writes to stdout deliberately, debug noise does not.
